...
const baseURL = config.app.buildAssetsDir;
...
const roomImages = computed(() => {
  ...
  const result = rooms.reduce((acc, roomId) => {
    acc[`room${roomId.toUpperCase()}`] = nums.reduce((obj, num) => {
      obj[num] = {
        desktop: `${baseURL}assets/images/room-${roomId}-${num}.png`,
        mobile: `${baseURL}assets/images/room-${roomId}-sm-${num}.png`,
      };
      return obj;
    }, {});
...
const roomData = await roomStore.roomData;
</script>
          ...
          >
            <div class="row">
              <div class="col-12 col-lg-7">
                ...
                >
                  <swiper-slide
                    v-for="(image, index) in item.imageUrlList"
                    :key="index"
                  >
                    <picture>
